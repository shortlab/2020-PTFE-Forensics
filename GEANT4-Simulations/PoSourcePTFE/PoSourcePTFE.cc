  ...
  if ( macro.size() ) {
    if (macro == "julieRun") {
      ...
      std::ostringstream commandOS;
        ...
        B4DetectorConstruction* detConstruction1 = new B4DetectorConstruction();
        detConstruction1->Construct();
        runManager->SetUserInitialization(detConstruction1);
        B4aActionInitialization* actionInitialization1 = new B4aActionInitialization(detConstruction1);
        ...
        //UImanager->ApplyCommand("/vis/modeling/trajectories/drawByParticleID-0/set alpha green");
        ...
        cout << command << endl;
        command = "/gps/ion 2 4 2";
        UImanager->ApplyCommand(command);
        cout << command << endl;

        // Po 210 decay
        command = "/gps/energy 5.305 MeV";
        UImanager->ApplyCommand(command);
        cout << command << endl;
        ...
        command = "/gps/pos/type Volume";
          UImanager->ApplyCommand(command);
          cout << command << endl;
        command = "/gps/pos/shape Para";
          UImanager->ApplyCommand(command);
          cout << command << endl;
        command = "/gps/pos/centre 0.0 0.0 0.0 cm";
          UImanager->ApplyCommand(command);
          cout << command << endl;
        command = "/gps/pos/halfx 1. cm";
          UImanager->ApplyCommand(command);
          cout << command << endl;
        command = "/gps/pos/halfy 0.5 cm";
          UImanager->ApplyCommand(command);
          cout << command << endl;
        command = "/gps/pos/halfz 0.000025 cm";
          UImanager->ApplyCommand(command);
          cout << command << endl;

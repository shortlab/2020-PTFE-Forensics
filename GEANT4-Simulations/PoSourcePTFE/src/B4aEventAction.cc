...
void B4aEventAction::EndOfEventAction(const G4Event* event)
{
    ...
    if(run) runNum = run->GetRunID();
            //cout << eID << " " << min(3000,(int)eventTotEdepDetector_keV) << endl;
            // note this is in kev
          runAction->detector_energy_deposited_histogram_keV[min(2000,(int)eventTotEdepDetector_keV)] += 1;
